...
const uint32_t sha256_initial_h[8] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
                                      0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};

const uint32_t sha256_round_k[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
    ...
    for (uint64_t cursor = 0; cursor * 64 < total; cursor++)
    {
        ...
        else
        {
            if (cursor * 64 < len)
            {
                ...
                if (size > 0)
                    memcpy(w, (uint8_t *)data + cursor * 64, size);
